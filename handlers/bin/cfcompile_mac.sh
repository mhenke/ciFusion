...
CFUSION_HOME=/Applications/ColdfusionMX7
...
J2EEJAR=/Applications/ColdfusionMX7/runtime/lib/jrun.jar

# This needs to point to the WEB-INF directory for ColdFusion.
WEBINF=/Applications/ColdfusionMX7/wwwroot/WEB-INF
...
case `uname` in
 ...
 Darwin)
        OS=Darwin
       /bin/sh -c "${_JAVACMD} -cp $CLASSES -Dcoldfusion.classPath=$CFUSION_HOME/lib/updates,$CFUSION_HOME/lib -Dcoldfusion.libPath=$CFUSION_HOME/lib coldfusion.tools.CommandLineInvoker Compiler -deploy -webinf $WEBINF -webroot $webroot -cfroot $CFUSION_HOME -srcdir $srcdir -deploydir $deploydir"
        ;;
 Linux)
        OS=Linux
...
case `uname` in
 SunOS)
	...
	Darwin)
                OS= Darwin
               /bin/sh -c "${_JAVACMD} -cp $CLASSES -Dcoldfusion.classPath=$CFUSION_HOME/lib/updates,$CFUSION_HOME/lib -Dcoldfusion.libPath=$CFUSION_HOME/lib coldfusion.tools.CommandLineInvoker Compiler -cfroot $CFUSION_HOME -webinf $WEBINF -webroot $webroot $dir"
        ;;
